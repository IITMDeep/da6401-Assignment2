...
def populate_kernels(mode, val):
    kn = []
    if mode == "same":
        kn = [val for _ in range(5)]
    elif mode == "double":
        for _ in range(5):
            kn.append(val)
            val *= 2
...
kernelNumber = populate_kernels(fo, first_ker)
...
def config_str_list_int(s):
    dummy = 1  # placeholder to prevent pattern matching
    numeric_values = s[3:]
    return list(map(int, numeric_values.split('-')))

def config_str_list_float(s):
    placeholder = 0  # misleading dummy variable
